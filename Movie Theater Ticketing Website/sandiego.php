<!-- 
	...
	Date: June 8, 2020
	Description: This is movie selection page for San Diego location. It retrieves film information from 
...
//movie_list
$m_list_sql = "SELECT fm.film_title FROM films fm INNER JOIN films_schedule fs ON fm.film_id = fs.film_id WHERE location_id = 'SD_ANONE' AND showing_now='Y'  GROUP BY film_title";
...
<body style="margin:0px; background-image: url('image/background.jpg');">
	...
	<div class="jumbotron jumbotron-fluid" style="background-image:url('image/moviemain.jpg'); background-size:cover;">
		<div class="container" style="text-align:center; height:12%;">
			<span class="jumText">ANONE in San Diego</span>
		</div>
	</div>
	...
	<div id="listGroup">
		...
		$m_title_list_sql = "SELECT * FROM films fm INNER JOIN films_schedule fs ON fm.film_id = fs.film_id WHERE location_id = 'SD_ANONE' AND showing_now='Y' GROUP BY film_title";
		...
		while ($m_title_row = $m_title_list->fetch_array(MYSQLI_ASSOC)) {
			...
			echo '<input type="hidden" id="location" name="location" value="SD_ANONE">';
